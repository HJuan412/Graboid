...
def plot_results(report, params, metric, prefix, ranks):
    for rk_idx, rk in enumerate(ranks):
        ...
        if len(rk_report) <= 1000:
            ...
            fig.savefig(prefix + f'{rk}.png', format='png', bbox_inches='tight')
            plt.close()
        else:
            for n_subplot, subplot_idx in enumerate(np.arange(0, len(rk_report), 1000)):
                ...
                fig.savefig(prefix + f'{rk}.{n_subplot + 1}.png', format='png', bbox_inches='tight')
                plt.close()
