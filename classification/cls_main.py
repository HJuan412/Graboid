...
def set_database(database):
    ...
    return DATA.DBASE_INFO[database] # database meta dict contains information of database files & directories

# load and map query file
def set_query(query_file, blast_db, query_dir, evalue=0.0005, dropoff=0.05, min_height=0.1, min_width=2, threads=1, qry_name='QUERY'):
    map_prefix = f'{query_dir}/{qry_name}'
    if mp.check_fasta(query_file) == 0:
        raise Exception(f'Error: Query file {query_file} is not a valid fasta file')
    map_matrix_file, map_acc_file, map_nrows, map_ncols = matrix.build_map(query_file, blast_db, map_prefix, evalue, threads)
    return map_matrix_file, map_acc_file, map_nrows, map_ncols

reads_file = '/media/hernan/disco_viejo/PROYECTOS/Maestria/g12_low_20K-reads.fa'
blast_db = 'test/test1/ref/db'
qry_dir = 'test'
map_matrix_file, map_acc_file, map_nrows, map_ncols = set_query(reads_file, blast_db, qry_dir)

#%% load maps and compare coverages

# TODO: you are working here

qry_map = np.load('test/QUERY__map.npz')
ref_map = np.load('test/test1/ref__map.npz')

qry_cov_norm = qry_map['coverage'] / qry_map['coverage'].max()
ref_cov_norm = ref_map['coverage'] / ref_map['coverage'].max()

min_cov = .95
selected_sites = np.arange(len(qry_cov_norm))[(qry_cov_norm >= min_cov) & (ref_cov_norm > 0)]

ref_mat = ref_map['matrix'][:,selected_sites]
#%%
def cls_main(query_file, work_dir, database, evalue=0.0005, dropoff=0.5, min_height=0.1, min_width=2, threads=1, qry_name='QUERY'):
    
    # prepare directories
    work_dir = re.sub('/$', '', work_dir)
    query_dir = f'{work_dir}/query'
    warn_dir = f'{work_dir}/warnings'
    # check database
    database = set_database(database)
    
    # check_query data
    map_file = set_query(query_file, database['blast_db'], query_dir, evalue, dropoff, min_height, min_width, threads, qry_name)
    # make work_dir
    return
#%%
class Classifier2:
