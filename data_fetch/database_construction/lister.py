...
def read_summ(summ_file, database):
    ...
    if len(accs) == 0:
        logger.warning(f'No records present in summary of the {database} database')
    return accs
#%% classes
class SummProcessor():
...
def generate_outfile(summ_files):
    for sample_file in summ_files.values():
        return sample_file.split('/')[-1].split('.')[0]

def build_list(summ_files, out_dir):
    # generate outfile
    out_file = generate_outfile(summ_files)
    out_file = f'{out_dir}/{out_file}.acc'
    # read summ_files
    acc_tabs = []
