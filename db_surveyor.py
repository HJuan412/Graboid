...
class Surveyor():
    ...
    def __init__(self, taxons, markers, type1tools, type2tools, out_dir, warn_dir):
        """
        Parameters
        ----------
        taxons : list
            List the taxons to survey for.
        markers : list
            List the markers to survey for.
        type1tools : list
    ...
    def attempt_dl(self, tooltype, ntries, taxon, marker = ''):
        ...
        tool = tooltype(taxon, marker, self.out_dir)
        dbase = tool.get_dbase()
        print(f'Surveying {dbase} for {taxon} {marker}')
        while tool.attempt < ntries and not tool.done:
            tool.survey()
        if not tool.done:
            print(f'Failed to get summary from {dbase} after {ntries} attempts.')
            self.warn_report = self.warn_report.append([tool.get_dbase, taxon, marker])
        else:
    ...
    def survey(self, ntries = 3):
        # Attempt to perform surveys for each taxon - marker - database trio.
        # ntries determines the number of attempts
        for taxon in self.taxons:
            for t1 in self.type1:
                self.attempt_dl(t1, ntries, taxon)
            
            for marker in self.markers:
                for t2 in self.type2:
                    self.attempt_dl(t2, ntries, taxon, marker)
        self.save_warn()